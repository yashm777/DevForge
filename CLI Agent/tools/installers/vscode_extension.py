...
def find_vscode_executable():
    ...
    if sys.platform == "win32":
        ...
        for path in common_paths:
            if os.path.exists(path):
                try:
                    # Run without shell for direct execution
    ...
    # Fallback to checking if 'code' is in the PATH, use shell=True here as a last resort for aliases
    try:
...
def get_installed_extensions():
    ...
    try:
        ...
        # IMPORTANT: Run without shell=True
        result = subprocess.run(command, check=True, capture_output=True, text=True)
        extensions = result.stdout.strip().split('\n')
        return set(ext for ext in extensions if ext) # Filter out empty strings
    except (FileNotFoundError, subprocess.CalledProcessError):
        return set()
...
def uninstall_extension(extension_id):
    ...
    if not any(extension_id.lower() == ext.lower() for ext in get_installed_extensions()):
    ...
    try:
        ...
        # IMPORTANT: Run without shell=True
        subprocess.run(uninstall_command, check=True, capture_output=True, text=True)
