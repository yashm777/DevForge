...
class TestGitConfiguratorLinux(unittest.TestCase):
    # Test if git is installed (should return True)
    @patch("subprocess.run")
    def test_is_git_installed_true(self, mock_run):
        mock_run.return_value.returncode = 0
        self.assertTrue(is_git_installed())

    # Test if git is not installed (should return False)
    @patch("subprocess.run")
    ...
    # Test SSH key generation when key does not exist
    ...
    # Test switching to a branch when the path exists
